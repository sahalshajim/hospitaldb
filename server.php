...
if (isset($_POST['login_user'])) {
  ...
  $usertype2 = mysqli_real_escape_string($db, $_POST['usertype']);
  ...
  if (count($errors) == 0) {
  	$password = $password;
  	$query = "SELECT * FROM users WHERE username='$username' AND password='$password' AND type='$usertype2'";
  	$results = mysqli_query($db, $query);
  	if (mysqli_num_rows($results) == 1) {
  	  $_SESSION['username'] = $username;
      $_SESSION['usertype'] = $usertype2;
  	  $_SESSION['success'] = "You are now logged in";
      if($usertype2 == 'Admin')
  	   header('location: admin.php');
       else if ($usertype2 == 'Doctor') {
